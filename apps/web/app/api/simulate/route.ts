...
function buildVideoPromptInstructions(age: number, gender: string, situation: string): string {
  return (
    ...
    "SEAMLESS LOOP DESIGN (always apply): The video must be designed for seamless looping. Start and end on the same type of slow micro-detail shot — a texture, a surface, a static object. Avoid any action with a clear beginning or end. No walking that starts and stops. No turning that completes. No gesture that resolves. Prefer slow drifting camera movements that have no obvious start or end point. The viewer must not be able to detect where the loop restarts. Think of it as a window into a continuous moment, not a clip with a beginning and end.\n" +
