...
function buildVideoPromptInstructions(age: number, _gender: string, situation: string): string {
  ...
  return (
    ...
    "THREATENING FACES IN SOCIAL INTERACTIONS: When there are people talking to or interacting with the camera (the protagonist), their faces must appear uncomfortably close — invading personal space, looming toward the camera. Their expressions should feel ambiguous and slightly threatening even if they are smiling. Eyes feel too intense, too focused. The proximity feels physically uncomfortable. Faces should fill a large portion of the frame when people are speaking or interacting. " +
    "NO TEXT IN VIDEO: Do not add any subtitles, captions, text overlays, watermarks, or written words anywhere in the video. The video must be purely visual with no text elements of any kind. " +
