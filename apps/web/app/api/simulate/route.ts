...
function buildVeoPrompt(
  ...
  if (classification.environment === 'E') {
    ...
    return `GoPro-style first-person eye-level shot at ${height}cm. Scene context: ${situation}. ${style.camera} ${style.focus} ${anchorInstruction} ${style.lighting} ${style.details}${modifierText}${antiHorror} Seamless 8-second loop: final frame identical to opening frame. Audio: ${audio}. Audio must match EXACTLY what you would hear in: ${situation} — not generic ambient, but the specific sounds of THIS scene amplified to equal overwhelming volume. No glitch effects. Photorealistic. No protagonist body visible. White Western European appearance. No masks. The scene must show exactly what is described in the situation. ABSOLUTE: Single continuous uncut shot.`;
  }
