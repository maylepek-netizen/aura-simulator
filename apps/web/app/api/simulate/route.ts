...
function buildVeoPrompt(
  ...
  if (classification.environment === 'B' || classification.environment === 'C') {
    ...
    if (hasPerson) {
      ...
      // No specific person - use environment-based approach like A
      return `GoPro-style first-person eye-level shot at ${height}cm. Scene context: ${situation}. Locked, incredibly still. Extreme close-up on one specific small detail that exists in this scene — its texture, color, material. Shallow depth of field f/1.2. Background exists but completely out of focus. Slight involuntary breathing tremor. ${style.lighting} Colors slightly oversaturated. Seamless 8-second loop. Audio: ${audio}. Audio must match EXACTLY what you would hear in: ${situation}. Photorealistic. No protagonist body visible. White Western European appearance. No masks.${modifierText} ABSOLUTE NON-NEGOTIABLE: ONE single unbroken take. No cuts. No fades. No transitions. STRICT: No AI glitches, no morphing, no digital artifacts.${thoughtsDirection}`;
    }
  }
