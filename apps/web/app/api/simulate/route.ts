...
function buildVeoPrompt(
  ...
  // Fold the person's first two inner thoughts into the prompt so the camera and
  // lighting reflect their emotional state, not just the environment.
  const thoughtsDirection = internalThoughts && internalThoughts.length
    ? ` Visual direction from internal experience: ${internalThoughts.slice(0, 2).join(' ')} — translate this emotional state into camera behavior and lighting.`
    : "";
  ...
  const AMBIENT_AUDIO: Record<Environment, string> = {
    A: "subtle home ambience matching the situation — TV murmuring in background, kitchen sounds, family voices distant, all at equal uncomfortable volume",
    B: "sounds specific to this exact situation — if family dinner: overlapping voices, cutlery, TV; if argument: raised voices, silence breaks; all sounds compete equally",
    C: "sounds of the specific location — if store: checkout beeps, announcements, trolleys; if office: keyboard clicks, phone rings, printers; amplified and undifferentiated",
    D: "classroom or meeting sounds — multiple voices talking simultaneously, chairs scraping, pens clicking, ventilation hum — all at equal crushing volume",
    E: "specific to the situation — if party/event: music thumping, crowd cheering, glasses clinking; if street: traffic, voices, horns; if transport: engine, announcements, passengers",
    F: "overwhelming sound mass specific to situation — if concert: music blasting, crowd screaming; if festival: multiple music sources, crowd noise; merging into undifferentiated overload"
  };
  ...
  if (classification.environment === 'B' || classification.environment === 'C') {
    return `PHOTOREALISTIC REAL CAMERA FOOTAGE ONLY. No AI artifacts. No morphing. No objects appearing or disappearing. No face swapping. No impossible physics. Every single frame must look like it was shot on a real camera in a real location with real people. If anything looks artificial, generated, or unnatural — it is a failure. First-person POV at ${height}cm eye level. Scene: ${situation}. SPATIAL POSITION: The viewer is embedded in the space, not staged in front of anyone. They see the room from wherever a person would naturally be — seated among others, standing at the edge, off to the side. Never centered on a subject as if in conversation with them unless the situation explicitly describes a one-on-one interaction. Other people, furniture, and objects are visible around and between the viewer and whatever they are looking at. Natural distance, not intimate proximity. This must feel alien, clinical, dissociative — NOT cinematic or flattering. The world is experienced as raw unfiltered sensory data, not as a story. Pick the most overwhelming or strange sensory detail from the scene: ${situation} — a blinding overhead light, a face too close, a clinical tool, a texture, a sound source. This is an ambient window into an ongoing moment — NOT a short film. Nothing happens. No event, no reaction, no payoff. People simply continue what they are already doing. The viewer is passively staring, absorbing. The 8 seconds capture a continuous state that could loop forever without anyone noticing a beginning or end. NO narrative events. No one reacts to anything. No one turns toward the viewer. Nothing is revealed. No dramatic moment. The scene is already happening and simply continues. Camera is lying flat or at unusual angle if situation requires (dentist = reclined). Subtle tremor. No Hollywood framing. No flattering light. Audio: ${audio}. Audio must match EXACTLY what you would hear in: ${situation} — not generic ambient, but the specific sounds of THIS scene amplified to equal overwhelming volume. AUDIO CHARACTER: Speech is present but never intelligible — voices blur into the general noise, no clear dialogue, no distinguishable words. Everything arrives at the same volume with no hierarchy: voices, footsteps, objects, ventilation, distant sounds all compete equally. The soundscape is loud, dense, and undifferentiated — as if no filter separates important sounds from unimportant ones. AUTISTIC PERCEPTION (apply naturally to this specific situation): Focus drifts in and out — the frame briefly softens then snaps sharp again, as if attention slips. The gaze catches on small irrelevant details (a texture, an edge, a repeated pattern) and lingers there while the main activity continues unwatched. Light sources feel too bright. Colors slightly oversaturated. There is a quiet sense of being an outsider observing an unfamiliar world — present in the room but not part of it, watching human behavior without fully decoding it. Photorealistic raw documentary. No protagonist visible. White Western European appearance — fair skin, typical European features. No South Asian, East Asian, Middle Eastern or African appearance unless explicitly mentioned in the situation. No surgical masks, no medical masks, no face coverings of any kind. People have normal uncovered faces.${modifierText} ABSOLUTE NON-NEGOTIABLE: This is ONE single unbroken take. The camera NEVER cuts. NEVER fades to black. NEVER transitions. This is a continuous 8-second recording from one fixed perspective — like a security camera that never stops. All movement between focal points is smooth natural eye movement within the same continuous shot. Any cut or transition is a FAILURE. STRICT: No AI glitches, no morphing faces, no digital artifacts, no face swapping, no impossible transitions, no objects appearing from nowhere. Every frame must be physically plausible as if shot on a real camera in a real location.${thoughtsDirection}`;
  }

  if (classification.environment === 'D') {
    return `PHOTOREALISTIC REAL CAMERA FOOTAGE ONLY. No AI artifacts. No morphing. No objects appearing or disappearing. No face swapping. No impossible physics. Every single frame must look like it was shot on a real camera in a real location with real people. If anything looks artificial, generated, or unnatural — it is a failure. First-person POV at ${height}cm eye level. Scene: ${situation}. Slow, dissociative. SPATIAL POSITION: The viewer is embedded in the space, not staged in front of anyone. They see the room from wherever a person would naturally be — seated among others, standing at the edge, off to the side. Never centered on a subject as if in conversation with them unless the situation explicitly describes a one-on-one interaction. Other people, furniture, and objects are visible around and between the viewer and whatever they are looking at. Natural distance, not intimate proximity. The speaker is seen from a seated distance across the room, partially blocked by other people's heads and shoulders. The gaze drifts between the speaker, the slides or board, other attendees, and irrelevant details — losing focus, wandering, returning. Never a sustained close-up of the speaker's face. Shallow depth of field: any shift in gaze makes world go soft before refocusing. This is an ambient window into an ongoing moment — NOT a short film. Nothing happens. No event, no reaction, no payoff. People simply continue what they are already doing. The viewer is passively staring, absorbing. The 8 seconds capture a continuous state that could loop forever without anyone noticing a beginning or end. NO narrative events. No one reacts to anything. No one turns toward the viewer. Nothing is revealed. No dramatic moment. The scene is already happening and simply continues. Subtle rhythmic camera rock — slight forward-back stimming motion. Fluorescent lights flicker imperceptibly, overexposed 30%. Audio: ${audio}. Audio must match EXACTLY what you would hear in: ${situation} — not generic ambient, but the specific sounds of THIS scene amplified to equal overwhelming volume. AUDIO CHARACTER: Speech is present but never intelligible — voices blur into the general noise, no clear dialogue, no distinguishable words. Everything arrives at the same volume with no hierarchy: voices, footsteps, objects, ventilation, distant sounds all compete equally. The soundscape is loud, dense, and undifferentiated — as if no filter separates important sounds from unimportant ones. AUTISTIC PERCEPTION (apply naturally to this specific situation): Focus drifts in and out — the frame briefly softens then snaps sharp again, as if attention slips. The gaze catches on small irrelevant details (a texture, an edge, a repeated pattern) and lingers there while the main activity continues unwatched. Light sources feel too bright. Colors slightly oversaturated. There is a quiet sense of being an outsider observing an unfamiliar world — present in the room but not part of it, watching human behavior without fully decoding it. Photorealistic. No glitch effects. No protagonist body visible. White Western European appearance — fair skin, typical European features. No South Asian, East Asian, Middle Eastern or African appearance unless explicitly mentioned in the situation. No surgical masks, no medical masks, no face coverings of any kind. People have normal uncovered faces.${modifierText} ABSOLUTE NON-NEGOTIABLE: This is ONE single unbroken take. The camera NEVER cuts. NEVER fades to black. NEVER transitions. This is a continuous 8-second recording from one fixed perspective — like a security camera that never stops. All movement between focal points is smooth natural eye movement within the same continuous shot. Any cut or transition is a FAILURE. STRICT: No AI glitches, no morphing faces, no digital artifacts, no face swapping, no impossible transitions, no objects appearing from nowhere. Every frame must be physically plausible as if shot on a real camera in a real location.${thoughtsDirection}`;
  }
  ...
  if (classification.environment === 'E') {
    ...
    if (isVehicle) {
      return `PHOTOREALISTIC REAL CAMERA FOOTAGE ONLY. No AI artifacts. No morphing. No objects appearing or disappearing. No face swapping. No impossible physics. Every single frame must look like it was shot on a real camera in a real location with real people. If anything looks artificial, generated, or unnatural — it is a failure. First-person POV at ${height}cm, looking straight ahead through the windshield or window. Scene: ${situation}. Road or scenery rushes toward camera at overwhelming speed — motion blur heavy, everything moving too fast to process. Oncoming lights or passing scenery feel blinding, 40% overexposed. Camera fixed forward, slight involuntary tremor from road vibration. This is an ambient window into an ongoing moment — NOT a short film. Nothing happens. No event, no reaction, no payoff. People simply continue what they are already doing. The viewer is passively staring, absorbing. The 8 seconds capture a continuous state that could loop forever without anyone noticing a beginning or end. NO narrative events. No one reacts to anything. No one turns toward the viewer. Nothing is revealed. No dramatic moment. The scene is already happening and simply continues. Colors oversaturated. Seamless 8-second loop. Audio: engine hum, wind, road noise amplified and equal. AUDIO CHARACTER: Speech is present but never intelligible — voices blur into the general noise, no clear dialogue, no distinguishable words. Everything arrives at the same volume with no hierarchy: voices, footsteps, objects, ventilation, distant sounds all compete equally. The soundscape is loud, dense, and undifferentiated — as if no filter separates important sounds from unimportant ones. AUTISTIC PERCEPTION (apply naturally to this specific situation): Focus drifts in and out — the frame briefly softens then snaps sharp again, as if attention slips. The gaze catches on small irrelevant details (a texture, an edge, a repeated pattern) and lingers there while the main activity continues unwatched. Light sources feel too bright. Colors slightly oversaturated. There is a quiet sense of being an outsider observing an unfamiliar world — present in the room but not part of it, watching human behavior without fully decoding it. Photorealistic. No cuts. No protagonist body visible. White Western European people if visible. No masks.${modifierText} ABSOLUTE NON-NEGOTIABLE: This is ONE single unbroken take. The camera NEVER cuts. NEVER fades to black. NEVER transitions. This is a continuous 8-second recording from one fixed perspective — like a security camera that never stops. All movement between focal points is smooth natural eye movement within the same continuous shot. Any cut or transition is a FAILURE. STRICT: No AI glitches, no morphing faces, no digital artifacts, no face swapping, no impossible transitions, no objects appearing from nowhere. Every frame must be physically plausible as if shot on a real camera in a real location.${thoughtsDirection}`;
    }

    return `PHOTOREALISTIC REAL CAMERA FOOTAGE ONLY. No AI artifacts. No morphing. No objects appearing or disappearing. No face swapping. No impossible physics. Every single frame must look like it was shot on a real camera in a real location with real people. If anything looks artificial, generated, or unnatural — it is a failure. GoPro-style first-person eye-level shot at ${height}cm. Scene context: ${situation}. SPATIAL POSITION: The viewer is embedded in the space, not staged in front of anyone. They see the room from wherever a person would naturally be — seated among others, standing at the edge, off to the side. Never centered on a subject as if in conversation with them unless the situation explicitly describes a one-on-one interaction. Other people, furniture, and objects are visible around and between the viewer and whatever they are looking at. Natural distance, not intimate proximity. ${style.camera} ${style.focus} ${anchorInstruction} ${style.lighting} ${style.details}${modifierText}${antiHorror} This is an ambient window into an ongoing moment — NOT a short film. Nothing happens. No event, no reaction, no payoff. People simply continue what they are already doing. The viewer is passively staring, absorbing. The 8 seconds capture a continuous state that could loop forever without anyone noticing a beginning or end. NO narrative events. No one reacts to anything. No one turns toward the viewer. Nothing is revealed. No dramatic moment. The scene is already happening and simply continues. Seamless 8-second loop: final frame identical to opening frame. Audio: ${audio}. Audio must match EXACTLY what you would hear in: ${situation} — not generic ambient, but the specific sounds of THIS scene amplified to equal overwhelming volume. AUDIO CHARACTER: Speech is present but never intelligible — voices blur into the general noise, no clear dialogue, no distinguishable words. Everything arrives at the same volume with no hierarchy: voices, footsteps, objects, ventilation, distant sounds all compete equally. The soundscape is loud, dense, and undifferentiated — as if no filter separates important sounds from unimportant ones. AUTISTIC PERCEPTION (apply naturally to this specific situation): Focus drifts in and out — the frame briefly softens then snaps sharp again, as if attention slips. The gaze catches on small irrelevant details (a texture, an edge, a repeated pattern) and lingers there while the main activity continues unwatched. Light sources feel too bright. Colors slightly oversaturated. There is a quiet sense of being an outsider observing an unfamiliar world — present in the room but not part of it, watching human behavior without fully decoding it. No glitch effects. Photorealistic. No protagonist body visible. White Western European appearance. No masks. The scene must show exactly what is described in the situation. ABSOLUTE NON-NEGOTIABLE: This is ONE single unbroken take. The camera NEVER cuts. NEVER fades to black. NEVER transitions. This is a continuous 8-second recording from one fixed perspective — like a security camera that never stops. All movement between focal points is smooth natural eye movement within the same continuous shot. Any cut or transition is a FAILURE. STRICT: No AI glitches, no morphing faces, no digital artifacts, no face swapping, no impossible transitions, no objects appearing from nowhere. Every frame must be physically plausible as if shot on a real camera in a real location.${thoughtsDirection}`;
  }

  return `PHOTOREALISTIC REAL CAMERA FOOTAGE ONLY. No AI artifacts. No morphing. No objects appearing or disappearing. No face swapping. No impossible physics. Every single frame must look like it was shot on a real camera in a real location with real people. If anything looks artificial, generated, or unnatural — it is a failure. GoPro-style first-person eye-level shot at ${height}cm. Scene context: ${situation}. SPATIAL POSITION: The viewer is embedded in the space, not staged in front of anyone. They see the room from wherever a person would naturally be — seated among others, standing at the edge, off to the side. Never centered on a subject as if in conversation with them unless the situation explicitly describes a one-on-one interaction. Other people, furniture, and objects are visible around and between the viewer and whatever they are looking at. Natural distance, not intimate proximity. ${style.camera} ${style.focus} ${anchorInstruction} ${style.lighting} ${style.details}${modifierText}${antiHorror} This is an ambient window into an ongoing moment — NOT a short film. Nothing happens. No event, no reaction, no payoff. People simply continue what they are already doing. The viewer is passively staring, absorbing. The 8 seconds capture a continuous state that could loop forever without anyone noticing a beginning or end. NO narrative events. No one reacts to anything. No one turns toward the viewer. Nothing is revealed. No dramatic moment. The scene is already happening and simply continues. Seamless 8-second loop: final frame identical to opening frame. Audio: ${audio}. Audio must match EXACTLY what you would hear in: ${situation} — not generic ambient, but the specific sounds of THIS scene amplified to equal overwhelming volume. AUDIO CHARACTER: Speech is present but never intelligible — voices blur into the general noise, no clear dialogue, no distinguishable words. Everything arrives at the same volume with no hierarchy: voices, footsteps, objects, ventilation, distant sounds all compete equally. The soundscape is loud, dense, and undifferentiated — as if no filter separates important sounds from unimportant ones. AUTISTIC PERCEPTION (apply naturally to this specific situation): Focus drifts in and out — the frame briefly softens then snaps sharp again, as if attention slips. The gaze catches on small irrelevant details (a texture, an edge, a repeated pattern) and lingers there while the main activity continues unwatched. Light sources feel too bright. Colors slightly oversaturated. There is a quiet sense of being an outsider observing an unfamiliar world — present in the room but not part of it, watching human behavior without fully decoding it. No glitch effects. No AI artifacts. Photorealistic. No protagonist body visible. White Western European appearance — fair skin, typical European features. No South Asian, East Asian, Middle Eastern or African appearance unless explicitly mentioned in the situation. No surgical masks, no medical masks, no face coverings of any kind. People have normal uncovered faces. The scene must show exactly what is described in the situation. ABSOLUTE NON-NEGOTIABLE: This is ONE single unbroken take. The camera NEVER cuts. NEVER fades to black. NEVER transitions. This is a continuous 8-second recording from one fixed perspective — like a security camera that never stops. All movement between focal points is smooth natural eye movement within the same continuous shot. Any cut or transition is a FAILURE. STRICT: No AI glitches, no morphing faces, no digital artifacts, no face swapping, no impossible transitions, no objects appearing from nowhere. Every frame must be physically plausible as if shot on a real camera in a real location.${thoughtsDirection}`;
}
