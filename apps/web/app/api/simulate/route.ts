...
function buildVeoPrompt(
  ...
  if (classification.environment === 'B' || classification.environment === 'C') {
    return `First-person POV at ${height}cm eye level. Scene: ${situation}. Slow, dissociative, slightly detached. Choose focal points that make sense for THIS specific situation — could be a person's face detail, an object in the room, a texture on the floor, something on a table, whatever exists naturally in: ${situation}. Shallow depth of field: world goes completely blurry between micro-gaze shifts. 0-3s: locked on one specific detail relevant to the situation — sharp close-up. 3-5s: slow involuntary drift to a different detail in the environment — could be object, texture, person, floor. 5-7s: 1-2 second processing delay then gaze drifts back, soft then snaps sharp. 7-8s: return to opening frame. Subtle rhythmic camera sway. Overexposed light sources. Colors slightly oversaturated on focal detail. Audio: ${audio}. Photorealistic. No glitch effects. No protagonist body, hands or face visible — camera IS their eyes. White light-skinned people.${modifierText}`;
  }
