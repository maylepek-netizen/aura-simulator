...
function buildVeoPrompt(
  ...
  if (classification.environment === 'B' || classification.environment === 'C') {
    return `First-person POV at ${height}cm eye level. Scene: ${situation}. This must feel alien, clinical, dissociative — NOT cinematic or flattering. The world is experienced as raw unfiltered sensory data, not as a story. Pick the most overwhelming or strange sensory detail from the scene: ${situation} — a blinding overhead light, a face too close with a mask, a clinical tool, a texture, a sound source. 0-3s: that overwhelming detail fills frame in extreme close-up, slightly too bright, colors too vivid. 3-5s: slow involuntary drift away — another detail in the environment, equally present. 5-7s: original overwhelming detail intrudes back into frame, too close, 1 second soft blur then sharp. 7-8s: back to start. Camera is lying flat or at unusual angle if situation requires (dentist = reclined). Subtle tremor. No Hollywood framing. No flattering light. Audio: ${audio}. Photorealistic raw documentary. No protagonist visible. White light-skinned people.${modifierText} ABSOLUTE: Single continuous uncut shot. No cuts. No fades. No transitions. No editing. One take, camera never stops recording. The movement between focal points must be smooth organic eye movement, not a cut.`;
  }
