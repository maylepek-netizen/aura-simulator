...
function buildVeoPrompt(
  ...
  if (classification.environment === 'D') {
    return `First-person POV at ${height}cm eye level. Scene: ${situation}. Slow, dissociative feel — not theatrical. Teacher or main person visible center frame, face 40-50% of frame. Hyper-attention on mouth movement — lips moving, jaw tension, expressions shifting and unreadable. Students or background people softly blurred, moving slowly in background bokeh. Camera has slight involuntary drift — almost imperceptible sway, like standing still but body barely moving. Intermittent soft focus: scene briefly goes slightly blurry then refocuses, like a dissociation moment. Harsh overhead fluorescent light overexposed 30% — feels clinical and too bright. Colors slightly oversaturated. Seamless 8-second loop: starts and ends on teacher face, same frame. Audio: ${audio}. No glitch effects. Photorealistic. No protagonist body visible. White light-skinned people.${modifierText}`;
  }
